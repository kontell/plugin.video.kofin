...
def find_markers(sig, ref, period_s=1.0, start_s=1.0, rate=RATE):
    ...
    for expect in range(int(start_s * rate), max(0, last), step):
        ...
        guard = np.concatenate([corr[lo:max(lo, idx - 4)], corr[min(hi, idx + 5):hi]])
...
def main(argv):
    ...
    ap.add_argument("--offset-samples", type=int, default=137,
    ...
    for row in manifest["tracks"]:
        ...
        if flag:
            ok = False
        print("   %-22s %7.1f  %7.1f  %7.1f%s" % (row["file"][:22], head, tail, full, flag))
    ...
    hits, snr = find_markers(sig, ref, manifest["chirp_period_s"], manifest["chirp_start_s"])
    ...
    print("   grid error         mean %+.3f  max |%.3f| samples (%.1f us)"
          % (err.mean(), np.abs(err).max(), np.abs(err).max() / RATE * 1e6))

    shifted = np.concatenate([np.zeros(args.offset_samples), sig])[:len(sig)]
    hits2, _ = find_markers(shifted, ref, manifest["chirp_period_s"], manifest["chirp_start_s"])
    n = min(len(hits), len(hits2))
    rec = np.median(hits2[:n] - hits[:n])
    print("   injected %d samples -> recovered %.3f  (error %.1f us)"
    ...
    for frac in (0.25, 0.5, 137.4):
        ...
        hits_f, _ = find_markers(shifted_f, ref, manifest["chirp_period_s"],
                                 manifest["chirp_start_s"])
        m = min(len(hits), len(hits_f))
        rec_f = np.median(hits_f[:m] - hits[:m])
        err_us = abs(rec_f - frac) / RATE * 1e6
        print("   injected %7.2f samples -> recovered %8.3f  (error %5.1f us)"
              % (frac, rec_f, err_us))
        if err_us > 1000:
            ok = False
    ...
    for name, codec in (("opus 128k", ["-c:a", "libopus", "-b:a", "128k"]),
        ...
        subprocess.run(["ffmpeg", "-y", "-v", "error", "-i", src] + codec + [tmp], check=True)
        lossy = load(tmp)
        h, s = find_markers(lossy, ref, manifest["chirp_period_s"], manifest["chirp_start_s"])
        m = min(len(h), len(hits))
        delta = np.median(h[:m] - hits[:m])
        spread = float(np.std(h[:m] - hits[:m]))
        status = "ok" if s > 8 and spread < 5 else "MARGINAL"
        print("   %-10s SNR %6.0fx  bias %+7.2f samples (%+.2f ms)  jitter %.2f samples  %s"
    ...
    refs = [reference_chirp(*r["chirp_hz"], ms=manifest["chirp_ms"])
    ...
    for row in manifest["tracks"]:
        ...
        if mark:
            ok = False
        print("   track %2d -> identified %2d   margin over runner-up %.2fx%s"
