...
**P0 (investigation, before code):** trace where `obj["LibraryId"]` comes from on each write path — the full-sync walk (library known) versus an incremental add of a brand-new show arriving by websocket — because it decides whether the incremental update leg can re-assert `media_folder` or must skip. Also confirm what `RemoveLibrary` does to a NULL-`media_folder` reference whose shared Kodi row died with the removed library, and whether the existing zero-orphan removal invariant already covers it.
...
**P3:** the spared arm heals instead of warning forever. `full_sync.prune` already confirms spared ids exist server-side (`full_sync.py:854-859`); for each, resolve its true library with one ancestors call (documented top-level route per the `/Users/{userId}` constraint — verify the exact 10.11 shape during implementation), then restamp `media_folder` if that library is whitelisted, else NULL. This converts the permanent `"kofin bugs"` warning (`full_sync.py:870-881`) into a converging heal, and it is also the **existing-install migration**: poisoned rows surface as spared on the next `UpdateLibrary` and get re-homed — no upgrade pass, no state-table change. The warning stays for ids the resolution cannot place.

---

## F3 — retry ceilings

**P1 — escalating recovery floor.** `schedule_recovery_prune` keeps its 3600s floor but the interval doubles each time an auto-scheduled recovery completes with `unapplied_count` still non-zero, capped at 86400s, and resets to 3600 when a drain reports zero unapplied. In-memory, on the Library manager beside `auto_prune_at` — a boot reset costs at most one 1h cycle before re-escalating, which is acceptable against the cost of a persistent attempt table (new kofin.db surface for a rate problem). The warning text gains the current interval so the log shows the escalation.
...
**P3 — the non-exit `LibraryException` drop.** `process_libraries` currently removes a library entry when the pass raised a non-exit `LibraryException`, even though the pass may not have landed (the in-file TODO at `full_sync.py:448-456`). Change it to keep the entry and count the pass as failed so the resume backoff owns the retry. Separate commit — it changes resume semantics and needs its own L1 around the exception taxonomy.
...
**P2 (optional):** stable ordering under degradation: when `/Library/MediaFolders` fails and the code degrades to views-only (`views.py:255-266`), reuse the previous `SortedViews` order for ids still present and append newcomers, rather than adopting the alternate endpoint's order wholesale — a transient 403 then no longer regenerates the tree twice per flip. Needs a check that `SortedViews` consumers tolerate retained ids the degraded answer lacked.
