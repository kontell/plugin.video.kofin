...
A live item (a TvChannel, or a source with `IsInfiniteStream`) is routed like any other video and its claim is stamped `Live` (`syncplay/utils.py::claim_is_live`; a public-bus claim spells it as no runtime). Its position is not the player clock: the add-on's state line reports the demux head on the **source clock** (`source_ms`, the container's own timestamps — a broadcast's PTS, one clock for every member of the group whatever moment each stream opened, and P0d of the pvr sync plan measured it surviving the server's remux and transcode per job) beside the player-clock reading for that head (`player_ms`), and the controller reads its own position as `getTime() + (source_ms − player_ms)` (`PlaybackController._position_ms`). The clock wraps every 2³³ ÷ 90 kHz, so a reading is placed on the group reference's cycle of it (`utils.unwrap_live_ms`), and a live group position on the wire sits ten days up (`LIVE_PTS_EPOCH_S`) so a member can tell a source-clock anchor from the session-time zero a proposer without the clock sends. A proposer with the clock anchors the group `LIVE_DELAY_S` (5 s) behind its own reading (`_forward_local_play` → `live_anchor_ms`): the headroom for a member whose stream lands later than the proposer's did (two devices opening the same provider playlist landed 2 s apart on the P4 gate). A live item is never seeked (`_seek_and_settle` returns at once). A member that lands ahead of the group by more than `LIVE_HOLD_MIN_MS` **holds**: it pauses for the excess, capped at `LIVE_HOLD_MAX_S`, and resumes through the dispatcher (`_live_hold`/`_live_resume`, from `_align_after_resume`) — exact on a timeshift buffer and seconds long, where pulsing the same distance off at the rate ceiling took minutes of slow motion; a hold yields to a group pause (the timer resumes nothing unless the reference is playing). Every live member's Ready report promises the group's own position: a Ready that reported the real reading — the proposer's, a delay ahead of its anchor — was one the server's ready barrier did not release on, and the group started on the 10 s wait timeout instead, distance every later joiner inherited.
...
- up to the pulse budget (*Largest error closed by tempo*, default 5 s, max 10 s): a pulse `(r, T)` with `(r − 1) × T` equal to the residual. The rate scales with the residual between 0.5 % and the *Fastest correction speed* (default 25 %), aiming at a 5 s pulse: 100 ms is 2 % for 5 s, 1 s is 20 % for 5 s, 2.5 s is 25 % for 10 s, 5 s is 25 % for 20 s, 10 s is 25 % for 40 s (`PULSE_MAX_S`). Above 5 % the rate is ramped in and out in 5 % steps every 250 ms — not seamless, just less of a jolt — and the hold is shortened by what the ramps already displace, so the schedule still moves exactly the residual;
- beyond it, and only when every sample in the window is beyond it: a seek, at rate 1.0, aimed ahead by this device's measured seek lag (restart time plus landing error, learned from every playing seek — the Tab restarts ~370 ms after a seek and lands 350 ms *early*, so a seek aimed at the group's current position leaves it ~700 ms behind), then a 30 s blackout during which a residual the seek left behind is closed by pulses after all, saturated at the rate ceiling for up to 40 s.
...
**Live items** take no seek at any residual: a residual beyond the budget runs saturated pulses for as long as it takes (a 30 s gap is three 40 s pulses at 25 %, about two minutes with settle), and only while the group is anchored on the source clock and this member can read its own — otherwise the window is left empty and the commands keep it tuned together. A forward pulse that moved less than half of what it asked for was starved — the demuxer ran out of feed, which is the live edge — and two in a row end fine sync for the item (`giving up: at the live edge`; the mismatch toast is not shown, the edge is not a fault). A pulse back is never starved: the buffer behind belongs to the stream. A starved pulse teaches the gain nothing.
...
3. The seek path: a ~1 s inject now pulses (it is inside the 5 s default; the 2026-08-22 +1 s seek was against a 300 ms floor). A leftover beyond the armed budget still seeks — the live driver change that lands a clean +6 s / +12 s is remaining work (`docs/syncplay-skip-calibration-plan.md`).
