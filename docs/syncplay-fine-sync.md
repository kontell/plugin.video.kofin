...
At group join (not at a re-join of the same group) the manager calls `begin()`, which arms only when all of these hold: `syncPlayTempo` on; inputstream.tempo installed and enabled (JSON-RPC `Addons.GetAddonDetails`); and either Kodi's `audiooutput.passthrough` off or `syncPlayTempoPassthrough` on. The reasons for not arming are logged as `[ syncplay/tempo ] fine sync …` and nothing else changes — the group behaves exactly as before.

On Kodi 22, with `syncPlayShortQueue` on, `videoplayer.queuetimesize` is set to 1 s for the session. The setting is read when the player object is constructed, so it applies to every play that starts after the join, which is all of them. The original value is written to the hidden `syncPlayQueueRestore` setting and put back at leave; a crash mid-session leaves that record behind, and `service/main.py` restores it at the next start. Kodi 21 has a fixed 8 s queue and is left alone. The queue depth in force is what the play route stamps as `queue_secs`, so the add-on reports time at the playing point.
...
- up to the pulse budget (300 ms — what 3 % closes in 10 s): a pulse `(r, T)` with `(r − 1) × T` equal to the residual. The rate scales with the residual between 0.5 % and 3 %, aiming at a 5 s pulse: 60 ms is 1.2 % for 5 s, 250 ms is 3 % for 8.3 s;
