...
AV1 video is routed only behind *Fine sync on AV1 video* (off): on a Pixel 7, a rate change followed by a seek wedged the AV1 hardware decoder (`dequeueInputBuffer failed` storm, frozen or corrupt picture with the clock running on) and a second run watchdog-rebooted the phone, while the Bravia's AV1 decoder took the same sequence cleanly; HEVC and h264 were clean on every device at 1.03×, 1.10× and 1.25×. The likely cause is on the add-on side — its post-seek probe loop frees the first packets after a seek, so a decoder is started mid-GOP (`[hevc] Could not find ref with POC` on the desktop after every seek through the add-on, none natively) — and is tracked there.
...
At group join (not at a re-join of the same group) the manager calls `begin()`, which arms only when `syncPlayTempo` is on and inputstream.tempo is installed and enabled (JSON-RPC `Addons.GetAddonDetails`). Audio passthrough is simply suspended for the session — the add-on decodes to PCM — and the setting's help says so; there is no second toggle. The reasons for not arming are logged as `[ syncplay/tempo ] fine sync …` and nothing else changes — the group behaves exactly as before.
