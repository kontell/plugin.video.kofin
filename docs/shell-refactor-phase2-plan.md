...
|---|---|
| **Date** | 2026-08-28 |
| **Source** | `docs/shell-refactor-assessment.md` §10 Tier 2, with the findings it stands on (§4 `service/player.py`, §5 `plugin/browse.py`/`play.py`, §2.4 the dispatch tables, §6 the `phase` machine) and the one `service → plugin` import phase 1 left by name (`latesubs → subtitles`, §2.1). Line references here are against `94c9338` (the `refactor/shell-phase1` tip). |
| **Branch** | `refactor/shell-phase2`, stacked on `refactor/shell-phase1` (PR #202), draft PR against it; one commit per item, each revertible on its own. Merge order stays #200 → #201 → #202 → this. |
