...
**Change.** `Service.onNotification` (`main.py:986`, ~145 lines, 29 branches) becomes two tables in the `settings_apply.py:59-72` shape: Kodi-bus `method → handler` and IPC `name → handler`, each handler a small method the table points at (shipped as *class-level method-name* maps resolved with `getattr` — the manager's shape rather than `settings_apply`'s `__init__`-built bound methods — because `test_sync_library.py` legitimately builds a `Service` with `__new__` and no `__init__`; the `__init__` version passed the targeted suites and failed that one, which CI on #205 caught after a background gate's exit code had been misread as green); the `DOWNLOAD_ADD`/`DOWNLOAD_CANCEL` payload parsing inside it moves to `downloads/wire.py` (new — it is the downloads wire format, and the manager's tests already pin the parsed shape). `SyncPlayManager._handle_group_update` (`manager.py:590`, 71 lines, 10 `gtype` arms) becomes a `gtype → handler` table the dispatcher reads. The guard order (nonce check at the door, the in-group gates) moves into the handlers unchanged.
