...
The "begin_walk → get_items → lock → construct writer → set_restore_point → per-item dialog.update → write → commit both → clear_restore_point" skeleton appears four times: `movies()` (`:685-729`), `tvshows_pass` inside `tvshows()` (`:784-829`), `musicvideos()` (`:861-899`), `boxsets()` (`:1179-1214`). `movies()` and `musicvideos()` are identical after name-normalisation. **Only the TV copy has `apply_or_skip`** (`:817`, `:731-765`), so a movie deleted mid-page — whose writer then calls `server.get_local_trailers` (`writers/movies.py:267`) — still aborts the whole library; `test_sync_full.py:58-66` records that exact bug as fixed for shows. `music()` (`:902-1007`) is a different shape again: one `music_database_lock` held for the whole library including network waits (`:904`), no restore points, no per-page commit.
