...
A claim with no `runtime_ticks` is a **live** claim — a channel, not an item. Positions on a live stream are session-relative, so the engine never seeks one and never reads its player clock as a group position. **A live channel is tuned, not fetched, so its claim belongs to a delegated provider** (see Register), not to a provider with a play template: a template would send followers to a URL of the engine's making while the member who pressed play watches the channel its own add-on tuned — two pipelines for one group, and the follower without the programme information, the live OSD or the timeshift bar its add-on would have given it. pvr.kofin claims live channels and catchup programmes alike under its own name for exactly that reason, telling them apart by the key, and executes both starts itself (measured on the rig, 2026-09-02: a follower on the template route opened `plugin://…?mode=play&id=` while the initiator played `pvr://…`; delegated, both open the same `pvr://` channel). With a `tempo` route whose add-on reports the **source clock** (inputstream.tempo ≥ 22.4.6 / 21.4.6 writes `source_ms` and `player_ms` in its state line: the demux head on the container's own timestamps, and the player-clock reading for it) the engine places the member on that clock — a broadcast's PTS, which every member's stream carries whatever moment it opened, and which survives the server's remux and transcode — and converges it by pulses alone: a proposer anchors the group 5 s behind its own reading, a member behind pulses forward until it meets the group or runs into its live edge (two starved pulses end fine sync for it), a member ahead holds still for the excess (its timeshift buffer keeps the picture) and pulses back the remainder. Without such a route, or in a group a member without the clock anchored on session time, members tune together on commands only. The source clock wraps every 2³³ ÷ 90 kHz; the engine compares on that circle, and a live group position on the wire is offset by ten days so it cannot be mistaken for session time.
