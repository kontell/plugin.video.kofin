...
def test_an_unreachable_download_worker_backs_the_spawn_path_off(ticking):
    ...
    assert manager.download_backoff_until == clock.now + DOWNLOAD_BACKOFF_SECONDS
...
def test_a_due_retry_runs_fast_sync_and_clears_itself(ticking):
    ...
    manager.retry_at = clock.datetime_now()

    manager.service()

    assert calls == ["fast"]
    assert manager.retry_at is None
...
def test_a_failed_retry_re_arms_with_backoff(ticking):
    ...
    manager.retry_at = clock.datetime_now()
    before = manager.retry_delay

    manager.service()

    assert manager.retry_at is not None
    assert manager.retry_at > clock.datetime_now()
    assert manager.retry_delay > before
...
def test_a_retry_is_not_due_until_its_time(ticking):
    ...
    clock.advance(-30)
    manager.retry_at = due

    manager.service()

    assert calls == [] and manager.retry_at == due
