...
def test_repair_reload_held_during_playback(builtins, monkeypatch):
    ...
    manager.refresher.flush_pending_reload()
    assert builtins == ["ReloadSkin()"]
...
def flush_settle(manager):
    """What the tick does with the settle: take what is due, refresh it."""
    settled = manager.refresher.settled()
    if settled:
        manager.refresh_libraries(settled)
...
def test_drain_refresh_waits_out_the_settle(builtins):
    ...
    manager.refresher.arm({"video"})
    flush_settle(manager)  # inside the settle: nothing fires
    assert builtins == []

    manager.refresher.arm({"music"})  # the second echo folds in

    manager.refresher.settle.due_at = datetime.now() - timedelta(seconds=1)
    flush_settle(manager)
