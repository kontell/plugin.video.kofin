"""Repoint policy: which rows move where, and the restore capture (W1.7).
...
when the writer-built filename is captured for restore, and the prune order.

Everything here is idempotent on purpose. "Already repointed" is the common
case, not an error: the writers re-assert a downloaded item's repoint after
their own pass rebuilds the row (plan W1.8), and the manager re-runs it at
start for drift — a library repair regenerates every row in writer shape,
and the next reconcile walks the done downloads back onto their files.
...
def mapping_for(jellyfin_id: str) -> Optional[Mapping]:
...
def repoint(download: store.Download, root: str) -> bool:
    ...
    the row.
    """
    mapping = mapping_for(download.jellyfin_id)
    ...
    with Database("video") as video:
        ...
        if current_name and current_name != filename:
            store.set_restore_filename(download.jellyfin_id, current_name)
